...
int audio_packet(AudioPacket **pAudioPacket, void *context) {
    return AVPacketPool::GetInstance()->getAudioPacket(pAudioPacket);
}

int video_packet(VideoPacket **pVideoPacket, void *context) {
    return AVPacketPool::GetInstance()->getVideoPacket(pVideoPacket);
}
...
int Mp4Muxer::Init(const char *path, int video_width, int video_height, int frame_rate, int video_bit_rate,
    ...
    if (ret != 0) {
        LOGE("alloc output context error: %d", av_err2str(ret));
        return ret;
    }
    ...
    RegisterAudioPacketCallback(audio_packet, this);
    RegisterVideoPacketCallback(video_packet, this);
    return 0;
}
...
int Mp4Muxer::WriteVideoFrame(AVFormatContext *oc, AVStream *st) {
    ...
    if (nalu_type == H264_NALU_TYPE_SEQUENCE_PARAMETER_SET) {
        ...
        if (ret < 0) {
            ...
            LOGE("avformat_write_header success %d", ret);
        }
    } else {
...
int Mp4Muxer::enqueue(uint8_t *data, int isAudio, int size, int pts) {
    int ret = 0;
    if (isAudio) {
        ret = AVPacketPool::GetInstance()->enqueueAudio(data, size, pts);
    } else {
        ret = AVPacketPool::GetInstance()->enqueueVideo(data, size, pts);
    }
    return ret;
}
